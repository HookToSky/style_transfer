...
CONTENT_WEIGHT=hp.HParam('content_weight', hp.Discrete([1e3, 1e4, 1e5])) 
LEARNING_RATE= hp.HParam('learning_rate', hp.Discrete([0.004, 0.005, 0.006]))
...
def run(content_targets, style_targets, params, opt):
    ...
    for n in range(12):
        with train_summary_writer.as_default():
            hp.hparams(params)
            tf.summary.image("Combined Image", img, step=n+1)
            ...
            for m in range(120):
                ...
                tf.summary.scalar('loss', train_loss.result(), step=n+1)
        ...
        for sw in STYLE_WEIGHT.domain.values:
            for tvw in TOTAL_VAR_WEIGHT.domain.values:
                for lr in LEARNING_RATE.domain.values:
                    ...
                    opt = tf.keras.optimizers.Adam(learning_rate=lr, beta_1=0.95, epsilon=1e-5)
